...
export async function createTestInvitation({ email, token, invitedById, expired }: CreateInvitationParams) {
  ...
  return runQuery((pool) =>
    pool.query(
      `INSERT INTO sub.invitations (public_id, token, email, first_name, last_name, role, status, invited_by_id, expires_at, created_at, updated_at)
...
export async function markInvitationAccepted(token: string) {
  return runQuery((pool) =>
    pool.query("UPDATE sub.invitations SET status = 'ACCEPTED', accepted_at = NOW() WHERE token = $1", [token]),
  )
}
...
export async function upsertTestUser(email: string, authProviderId: string) {
  return runQuery((pool) =>
    pool.query(
      `INSERT INTO sub.users (public_id, auth_provider_id, email, first_name, last_name, role, is_active, is_email_verified, created_at, updated_at)
...
export async function getTestUserId(email: string): Promise<number | undefined> {
  return runQuery(async (pool) => {
    const result = await pool.query('SELECT id FROM sub.users WHERE email = $1 AND deleted_at IS NULL', [email])
...
export async function cleanupTestUser(email: string) {
  return runQuery((pool) =>
    pool.query('UPDATE sub.users SET deleted_at = NOW() WHERE email = $1 AND deleted_at IS NULL', [email]),
  )
}

export async function cleanupTestInvitation(token: string) {
  return runQuery((pool) =>
    pool.query('UPDATE sub.invitations SET deleted_at = NOW() WHERE token = $1 AND deleted_at IS NULL', [token]),
  )
}
...
export async function createTestTeam(name: string): Promise<number> {
  return runQuery(async (pool) => {
    const result = await pool.query(
      `INSERT INTO sub.teams (public_id, name, created_at, updated_at)
       VALUES ($1, $2, NOW(), NOW())
       RETURNING id`,
      [randomUUID(), name],
    )
...
export async function createTestTeamMember(teamId: number, userId: number): Promise<number> {
  return runQuery(async (pool) => {
    const result = await pool.query(
      `INSERT INTO sub.team_members (team_id, user_id, created_at, updated_at)
       VALUES ($1, $2, NOW(), NOW())
       RETURNING id`,
      [teamId, userId],
    )
...
export async function cleanupTestTeam(teamId: number) {
  return runQuery(async (pool) => {
    await pool.query('UPDATE sub.team_members SET deleted_at = NOW() WHERE team_id = $1 AND deleted_at IS NULL', [
      teamId,
    ])
    await pool.query('UPDATE sub.teams SET deleted_at = NOW() WHERE id = $1 AND deleted_at IS NULL', [teamId])
  })
}
...
export async function createTestStudent({ name, email, token, expired }: CreateTestStudentParams): Promise<number> {
  ...
  return runQuery(async (pool) => {
    const result = await pool.query(
      `INSERT INTO sub.students (public_id, name, email, token, token_expires_at, created_at, updated_at)
...
export async function createTestVideo(title: string): Promise<number> {
  return runQuery(async (pool) => {
    const result = await pool.query(
      `INSERT INTO sub.videos (public_id, title, category, video_url, is_published, created_at, updated_at)
...
export async function cleanupTestStudent(studentId: number) {
  return runQuery(async (pool) => {
    await pool.query('DELETE FROM sub.watch_events WHERE student_id = $1', [studentId])
    await pool.query('UPDATE sub.students SET deleted_at = NOW() WHERE id = $1 AND deleted_at IS NULL', [studentId])
  })
}

export async function cleanupTestVideo(videoId: number) {
  return runQuery(async (pool) => {
    await pool.query('DELETE FROM sub.watch_events WHERE video_id = $1', [videoId])
    await pool.query('UPDATE sub.videos SET deleted_at = NOW() WHERE id = $1 AND deleted_at IS NULL', [videoId])
  })
}
